...
import com.example.goodNeighbor3.stream.StreamTestCodeVerifier3.AnimeNameAndCountry;

public class StreamTestCodeVerifier4 {
	...
	public static Long problem5() {
		return StreamTestDataSupplier.getAnimeList().stream().filter(it -> it.viewCount > 100)
				.filter(it -> it.revenue < 200).count();
	}
	...
	static class AnimeNameAndCountry {
		String name;
		String country;

		public AnimeNameAndCountry(String name, String country) {
			super();
			this.name = name;
	...
	public static List<AnimeNameAndCountry> problem7(){
		return StreamTestDataSupplier.getAnimeList().stream()
				.filter(it -> it.revenue >= 500)
				.map(it -> new AnimeNameAndCountry(it.name, it.country))
				.toList();
	}
