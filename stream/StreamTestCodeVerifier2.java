...
public class StreamTestCodeVerifier2 {
	...
	public static Long problem5() {
		return StreamTestDataSupplier.getAnimeList().stream().filter(it -> it.country.equals("일본"))
				.filter(it -> it.viewCount > 100).filter(it -> it.revenue < 200).count();
	}

	// 6번문제 : 제조국이 일본인 모든 애니메이션의 제목을 주세요.

	public static List<String> problem6() {
		return StreamTestDataSupplier.getAnimeList().stream().filter(it -> it.country.equals("일본")).map(it -> it.name)
				.toList();
	}

	// 7번문제 : 매출액이 500 이상인 모든 애니메이션의 이름과, 제조국을 주세요.

	public static List<ListAnimeNameAndCountry> problem7() {
		return StreamTestDataSupplier.getAnimeList().stream().filter(it -> it.revenue >= 500)
				.map(it -> new ListAnimeNameAndCountry(it.name, it.country)).toList();
	}

	static class ListAnimeNameAndCountry {
		String name;
		String country;

		public ListAnimeNameAndCountry(String name, String country) {
			super();
			this.name = name;
	...
	public static List<String> problem8() {
		return StreamTestDataSupplier.getAnimeList().stream()
